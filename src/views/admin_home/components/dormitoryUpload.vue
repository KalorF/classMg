<template>
  <div class="dormitory_box">
    <!-- 宿舍情况表单 -->
    <el-table
    :data="dormList.slice((currentPage-1)*pagesize,currentPage*pagesize)"
    style="width: 100%">
      <el-table-column
        label="姓名"
        width="180">
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button
            ...
            size="mini"
            type="success"
            @click="handledwonload(scope.$index, scope.row)">下载</el-button>
          <el-button size="mini" v-else>无</el-button>
        </template>
      </el-table-column>
    </el-table>
     <!-- 分页 -->
...
export default {
  ...
  methods: {
    ...
    formData () {
      const vm = this
      vm.dormList.map(item => {
        if (item.isUploadWork === 0) {
          vm.$set(item, 'status', '已上传')
        } else {
          vm.$set(item, 'status', '未上传')
        }
      })
    },
